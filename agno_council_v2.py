...
async def deliberate_query(query: str, verbose: bool = False):
    """Run a single deliberation on a custom query."""
    council = AgnoCouncilV2(enable_dgm=True, enable_streaming=False)
    council.activate()
    ...
    # Print the final answer (synthesis of all members)
    if response.final_answer:
        print(response.final_answer)
    elif response.member_contributions:
        for name, content in response.member_contributions.items():
...
async def show_status():
    """Show council status without deliberating."""
    council = AgnoCouncilV2(enable_dgm=True, enable_streaming=False)
    council.activate()
    stats = council.get_stats()
    print("AGNO COUNCIL v2 — Status")
    print("=" * 40)
    for key, value in stats.items():
        print(f"  {key}: {value}")
    print()
    ...
    for name, member in council.members.items():
        ...
        print(f"  {name} ({member.role}): {status}")
    council.deactivate()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="AGNO Council v2 — Multi-Agent Deliberation System",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="Examples:\n"
               "  python agno_council_v2.py --deliberate 'What is the R_V metric?'\n"
               "  python agno_council_v2.py --deliberate 'Analyze DGC architecture' -v\n"
               "  python agno_council_v2.py --status\n"
               "  python agno_council_v2.py --demo\n"
    )
    parser.add_argument(
        "--deliberate", "-d",
        type=str,
        help="Custom query for council deliberation"
    )
    parser.add_argument(
        "--status", "-s",
        action="store_true",
    ...
    parser.add_argument(
        "--demo",
        action="store_true",
        help="Run the full demo (original behavior)"
    )
    parser.add_argument(
        "--verbose", "-v",
        action="store_true",
        help="Show detailed output (confidence, timing, gates)"
    )
    ...
    if args.status:
        ...
        asyncio.run(deliberate_query(args.deliberate, verbose=args.verbose))
    elif args.demo:
        asyncio.run(demo())
    else:
        parser.print_help()
